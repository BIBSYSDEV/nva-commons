...
public class DoesNotHaveNullOrEmptyFields<T> extends BaseMatcher<T> {
  ...
  public void describeMismatch(Object item, Description description) {
    ...
    description.appendText("The following fields were found empty:").appendText(emptyFieldNames);
  }

  private boolean assertThatNoPublicFieldIsNull(Object input) {
    Stream<PropertyDescriptor> properties = retrieveProperties(input);
    emptyFields =
        properties.map(prop -> readPropertyValue(prop, input)).filter(this::isEmpty).toList();
  ...
  private Stream<PropertyDescriptor> retrieveProperties(Object input) {
    try {
      return Arrays.stream(
          Introspector.getBeanInfo(input.getClass(), Object.class).getPropertyDescriptors());
