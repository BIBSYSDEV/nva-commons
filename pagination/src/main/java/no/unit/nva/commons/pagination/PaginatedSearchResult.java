...
public final class PaginatedSearchResult<T> {
  ...
  public static <T> PaginatedSearchResult<T> create(
      URI baseUri, int queryOffset, int querySize, int totalHits, List<T> hits)
  ...
  public static <T> PaginatedSearchResult<T> create(
    ...
    var nextResults =
        calculateNextResults(
            queryOffset, querySize, totalHits, hits.size(), baseUri, queryParameters);
    var previousResults =
        calculatePreviousResults(queryOffset, totalHits, querySize, baseUri, queryParameters);

    return new PaginatedSearchResult<>(
        selfUri, totalHits, nextResults, previousResults, hits, aggregations);
  }
  ...
  private static URI calculateNextResults(
      int queryOffset,
      int querySize,
      int totalHits,
      int numberOfHits,
  ...
  private static URI calculatePreviousResults(
      int queryOffset, int totalHits, int querySize, URI baseUri, Map<String, String> queryParams) {
    return isFirstPage(queryOffset)
        ? null
        : generatePreviousResult(queryOffset, totalHits, querySize, baseUri, queryParams);
  }

  private static URI generatePreviousResult(
      int queryOffset, int totalHits, int querySize, URI baseUri, Map<String, String> queryParams) {
    var previousOffset =
        queryOffset < totalHits
  ...
  private static URI generateSelfUri(
      URI baseUri, int queryOffset, int querySize, Map<String, String> queryParams) {
  ...
  private static void validateOffsetAndSize(int offset, int size)
      throws UnprocessableContentException {
