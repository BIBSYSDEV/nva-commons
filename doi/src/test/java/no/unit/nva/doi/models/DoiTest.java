...
class DoiTest {
  ...
  void shouldReturnUriWhenCreatedWithHostAndDoiPathExtractedFromJavasUriClass() {
    ...
    var doi =
        Doi.fromDoiIdentifier(
            Doi.DEFAULT_HOST, extractedPathDirectlyFromUriContainingPathSeparator);
    var expectedUri =
        URI.create(DEFAULT_DOI_URI_PREFIX + extractedPathDirectlyFromUriContainingPathSeparator);
  ...
  void shouldTransformNonStandardDoiFormatsToStandardDoiFormat(String nonStandardUri)
  ...
  void shouldReturnExpectedUriWhenCustomHostPrefixAndSuffixAreSupplied() {
    ...
    var prefix =
        UnixPath.of(randomDoi().getPath()).removeRoot().getParent().orElseThrow().toString();
  ...
  @ParameterizedTest(
      name = "should return standard https URI when input is old-style presentation doi")
  @ValueSource(
      strings = {"doi:10.1000/182", "doc:10.1000/182", "DOI:10.1000/182", "DOC:10.1000/182"})
  ...
  private URI createExpectedSandboxDoiUri(URI seedValue, String hostOfSandboxEnvironment)
      throws URISyntaxException {
    return new URI(
        UriWrapper.HTTPS, hostOfSandboxEnvironment, seedValue.getPath(), DoiTest.EMPTY_FRAGMENT);
  }
