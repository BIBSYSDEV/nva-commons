...
public final class DoiSuppliers {
  ...
  public static Stream<DoiInput> validDois() throws URISyntaxException {
    ...
    DoiInput withCustomaryPrefixNonUriDoi =
        createDoiInput(CUSTOMARY_DOI_PREFIX + doiValue, expectedUri);
  ...
  private static String randomDoiString() {
    String prefixSecondPart =
        RandomStringUtils.randomNumeric(MIN_ARBITRARY_LENGTH, MAX_ARBITRARY_LENGTH);
  ...
  private static String randomDoiSuffix() {
    return IntStream.range(1, 2 + RANDOM.nextInt(4))
        .boxed()
        .map(
            ignored ->
                RandomStringUtils.randomAlphanumeric(MIN_ARBITRARY_LENGTH, MAX_ARBITRARY_LENGTH))
