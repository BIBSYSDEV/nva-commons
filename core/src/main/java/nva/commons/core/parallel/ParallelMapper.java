...
/**
 * {@link ParallelMapper} is a mapper for processing multiple items where the processing of each
 ...
 *
 * <p>{@code ParallelMapper} takes as input a {@link Collection} of inputs of class I, and a mapping
 * function {@link Function} that maps an object I to an object O. When {@link ParallelMapper#map()}
 * is called, it applies the mapping function to all the Collection items. The successful mappings
 * are available thought the method {@link ParallelMapper#getSuccesses()} and the failures through
 * the method {@link ParallelMapper#getExceptions()}.
 *
 * <p>The class provides also a {@link ParallelMapper#getCompleted()} method for the mappings that
 * completed either successfully or unsuccessfully. For more details about this check {@link
 * Future#isDone()}.
 *
 * <p>Example:
 *
 ...
 * </pre>
 *
 * <p>{@link ParallelMapper} accepts also another parameter: batchSize. This indicates the size of
 ...
 *
 * <p>For example, if the input has 10.000 elements and the batch size is 1.000, all 1.000 elements
 * of the first batch are going to be processed in parallel, and then the second batch is going to
 * be executed, etc. {@code ParallelMapper} creates one execution thread per item in a batch and
...
public class ParallelMapper<I, O> {
  ...
  public List<O> getSuccesses() {
    return getCompleted().filter(Try::isSuccess).map(Try::orElseThrow).toList();
  }
  ...
  private Stream<Try<O>> getCompleted() {
    return futures.stream().parallel().filter(Future::isDone).map(attempt(Future::get));
  }
  ...
  private Callable<O> toCallable(I input) {
    return () ->
        attempt(() -> mappingFunction.apply(input))
            .orElseThrow(fail -> captureAllExceptionsAndAddInputObject(input, fail));
  }

  private ParallelExecutionException captureAllExceptionsAndAddInputObject(
      I input, Failure<O> fail) {
