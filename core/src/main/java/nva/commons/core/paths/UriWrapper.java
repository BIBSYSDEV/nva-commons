...
public class UriWrapper {
  ...
  private static URI createUriWithSchemeAndHost(String scheme, String host, int port) {
    ...
    return attempt(() -> createUriFromHostUri(scheme, host, port))
  ...
  private static List<NameValuePair> extractKeyValuePairs(String query) {
    return Arrays.stream(splitKeyValuePairs(query)).map(UriWrapper::createNameValuePair).toList();
  }
  ...
  private static NameValuePair createNameValuePair(String pairString) {
    ...
    return new BasicNameValuePair(
        decode(pairString.substring(0, assignation)),
  ...
  private List<NameValuePair> extractUriQueryParameters() {
    ...
    return nonNull(query) ? extractKeyValuePairs(query) : Collections.emptyList();
  }
}
