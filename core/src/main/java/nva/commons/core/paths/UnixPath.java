...
public final class UnixPath {
  ...
  public static UnixPath of(String... path) {
    ...
    List<String> pathElementsList = addRootIfPresentInOriginalPath(pathElements, path).toList();
    return pathIsEmpty(pathElementsList) ? EMPTY_PATH : new UnixPath(pathElementsList);
  }
  ...
  public UnixPath removeRoot() {
    return isAbsolute() ? new UnixPath(this.path.subList(1, path.size())) : this;
  }
  ...
  private static Stream<String> addRootIfPresentInOriginalPath(
      Stream<String> pathElements, String... path) {
