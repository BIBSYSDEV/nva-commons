...
class ParallelMapperTest {
  ...
  public void parallelMapperReturnsFunctionResultsOnSetOfInputs() throws InterruptedException {
    ...
    ParallelMapper<Integer, String> mapper =
        new ParallelMapper<>(inputs, this::integerToString).map();
  ...
  public void parallelMapperAcceptsStreamsAsInput() throws InterruptedException {
    ...
    ParallelMapper<Integer, String> mapper =
        new ParallelMapper<>(inputs.stream(), this::integerToString).map();
    verifyParallelMapperTransformsInputObjects(mapper, inputs);
  }
  ...
  public void parallelMapperReturnsExceptionsForFailedExecutions() throws InterruptedException {
    List<Integer> inputs = sampleInputs(100);
    ParallelMapper<Integer, String> mapper =
        new ParallelMapper<>(inputs, this::failingIntegerToString).map();

    List<ParallelExecutionException> actualExceptions = mapper.getExceptions();

    List<String> actualExceptionMessages =
        actualExceptions.stream().map(Throwable::getMessage).toList();
    List<String> expectedExceptionMessages =
        inputs.stream().map(this::constructExpectedMessage).toList();
    assertThat(actualExceptionMessages, is(equalTo(expectedExceptionMessages)));
  }

  @Test
  public void parallelMapperReturnsExceptionsContainingTheFailingInputs()
      throws InterruptedException {
    List<Integer> inputs = sampleInputs(100);
    ParallelMapper<Integer, String> mapper =
        new ParallelMapper<>(inputs, this::failingIntegerToString).map();

    List<Integer> regeneratedInputs =
        mapper.getExceptions().stream()
  ...
  void parallelMapperThrowsNoExceptionForVeryLargeInputs() throws InterruptedException {
    List<Integer> input =
        sampleInputs(NUMBER_OF_INPUTS_WITH_TOTAL_FOOTPRINT_LARGER_THAN_AVAILABLE_MEMORY);
    ParallelMapper<Integer, String> mapper =
  ...
  private void verifyParallelMapperTransformsInputObjects(
      ParallelMapper<Integer, String> mapper, List<Integer> inputs) {
