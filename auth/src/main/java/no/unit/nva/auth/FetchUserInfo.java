...
public class FetchUserInfo {
  ...
  public FetchUserInfo(
      HttpClient httpClient, Supplier<URI> cognitoUri, String authorizationHeader) {
  ...
  public CognitoUserInfo fetch() {
    var request =
        HttpRequest.newBuilder(cognitoUri.get())
  ...
  private HttpResponse<String> responseIsSuccessful(HttpResponse<String> response) {
    if (HttpURLConnection.HTTP_OK != response.statusCode()) {
      LOGGER.error(
          "Failed to look up UserInfo from Cognito. Got status code {} with body '{}'",
          response.statusCode(),
          response.body());
