...
class S3DriverTest {
  ...
  @Test
  void
      shouldReturnResultContainingPartialFileListAndNewListStartingPointAndSignForTerminatingListing()
          throws IOException {
    ...
    ListingResult secondBatch =
        s3Driver.listFiles(UnixPath.of(SOME_PATH), firstFilePath.toString(), 1);
    assertThat(
        secondBatch.getListingStartingPoint(), is(equalTo(MARKER_INDICATING_END_OF_LISTING)));
  ...
  void shouldStoreAllContentUnderSpecifiedPathRemovingRoot(String pathPrefix) throws IOException {
    List<String> input = new ArrayList<>();
    for (int i = 0; i < LARGE_NUMBER_OF_INPUTS; i++) {
      input.add(longText());
    }
    ...
    List<String> actualContent =
        new BufferedReader(new InputStreamReader(compressedContent)).lines().toList();
    assertThat(actualContent, is(equalTo(input)));
    assertThat(
        toS3Path(fileLocation).toString(), startsWith(expectedFolderNeverContainsRootFolder));
  }
  ...
  void shouldBeAbleToDecodeStringInEncodingOtherThanUtf8() {
    ...
    var request = PutObjectRequest.builder().bucket(SAMPLE_BUCKET).key(filename).build();
