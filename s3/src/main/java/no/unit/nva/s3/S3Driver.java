...
public class S3Driver {
  ...
  public static S3ClientBuilder defaultS3Client() {
    Region region =
        ENVIRONMENT.readEnvOpt(AWS_REGION_ENV_VARIABLE).map(Region::of).orElse(Region.EU_WEST_1);
    return S3Client.builder().region(region).httpClient(httpClientForConcurrentQueries());
  }
  ...
  public List<String> getFiles(UnixPath folder) {
    return listAllFiles(folder).stream().map(this::getFile).toList();
  }
   ...
   * from the {@code listingStartingPoint} if is not null or from the beginning if it is null. After
   * a call the next starting point can be acquired by the {@link ListingResult}
   *
   ...
   * @param responseSize The number of filenames returned in each batch. Max size determined by S3
   *     is 1000.
  ...
  public ListingResult listFiles(UnixPath folder, String listingStartingPoint, int responseSize) {
    ...
    return new ListingResult(
        files, listingResult.nextContinuationToken(), listingResult.isTruncated());
  }
  ...
  private UnixPath calculateListingFolder(UnixPath folder) {
    return isNull(folder) || folder.isEmptyPath() || folder.isRoot() ? UnixPath.EMPTY_PATH : folder;
  }
  ...
  private String processPath(UnixPath s3Folder) {
    ...
    return unixPath.startsWith(UNIX_SEPARATOR) ? unixPath.substring(REMOVE_ROOT) : unixPath;
  }
  ...
  private ListObjectsV2Response fetchNewResultsBatch(
      UnixPath folder, String listingStartingPoint, int responseSize) {
  ...
  private GetObjectRequest createGetObjectRequest(UnixPath file) {
    return GetObjectRequest.builder().bucket(bucketName).key(file.toString()).build();
  }

  private DeleteObjectRequest createDeleteObjectRequest(UnixPath filename) {
    return DeleteObjectRequest.builder().bucket(bucketName).key(filename.toString()).build();
  }

  private List<UnixPath> extractResultsFromResponse(ListObjectsV2Response result) {
    return result.contents().stream().map(S3Object::key).map(UnixPath::of).toList();
  }

  private ListObjectsV2Request requestForListingFiles(
      UnixPath folder, String startingPoint, int responseSize) {
  ...
  private PutObjectRequest newPutObjectRequest(UnixPath fullPath) {
    return PutObjectRequest.builder().bucket(bucketName).key(fullPath.toString()).build();
  }
}
