...
public class GatewayResponse<T> implements Serializable {
  ...
  @JacocoGenerated
  public GatewayResponse(
      T body, Map<String, String> headers, int statusCode, ObjectMapper objectMapper)
   ...
   */
  public static <T> GatewayResponse<T> of(ByteArrayOutputStream outputStream)
  ...
  /**
   * Create GatewayResponse object from an output stream. Used when we call the method {@code
   * handleRequest()} of a Handler directly, and we want to read the output.
   *
   ...
   */
  public static <T> GatewayResponse<T> fromOutputStream(
      ByteArrayOutputStream outputStream, Class<T> className) throws JsonProcessingException {
    var json = outputStream.toString(StandardCharsets.UTF_8);
    return fromString(json, className);
  }

  /**
   * Create GatewayResponse object from a String. Used when we call the method {@code
   * handleRequest()} of a Handler directly and we want to read the output. Usually the String is
   * the output of an OutputStream.
   *
  ...
  private static <T> GatewayResponse<T> constructGatewayResponseWithStringBody(
    ...
    return (GatewayResponse<T>)
        attempt(
                () ->
                    new GatewayResponse(
                        body, headers, statusCode, isBase64Encoded, defaultRestObjectMapper))
            .orElseThrow();
  }
}
