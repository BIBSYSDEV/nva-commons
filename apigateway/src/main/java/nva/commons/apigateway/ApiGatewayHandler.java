...
public abstract class ApiGatewayHandler<I, O> extends RestRequestHandler<I, O> {
  ...
  /**
   * This is the message for the success case. Sends a JSON string containing the response that
   * APIGateway will send to the user.
   *
  ...
  protected void writeOutput(I input, O output, RequestInfo requestInfo)
    ...
    try (BufferedWriter writer =
      ...
      var gatewayResponse =
          new GatewayResponse<String>(serializedOutput, headers, statusCode, isBase64Encoded);
  ...
  /**
   * Sends a message to ApiGateway and to the API-client, in case of failure caused by an
   * ApiGatewayException (predicted exception). This method can be overriden for richer status
   * codes, but in the general case it should not be neccessary/
   *
   * @param input the input object of class I
   ...
   */
  @Override
  protected void writeExpectedFailure(I input, ApiGatewayException exception, String requestId)
      throws IOException {
    try {
  ...
  /**
   * Sends a message to ApiGateway and to the API-client in case of failure caused by an Exception
   * that is not ApiGatewayException (unpredicted exception). This method can be overriden for
   * richer status codes, but in the general case it should not be necessary.
   *
   * @param input the input object of class I
   * @param exception the exception
   * @throws IOException when serializing fails
  ...
  /**
   * Get the ObjectMapper to use for the given MediaType. Defaults to defaultRestObjectMapper if no
   * other ObjectMapper is found.
   *
   ...
   */
  protected ObjectMapper getObjectMapper(RequestInfo requestInfo)
  ...
  /**
   * If you want to override this method, maybe better to override the {@link
   * ApiGatewayHandler#defaultHeaders(RequestInfo requestInfo)}.
   *
  ...
  private GatewayResponse<Void> createRedirectResponse(RedirectException exception)
    ...
    var responseHeaders =
        Map.of(
            ...
            exception.getLocation().toString(),
            ACCESS_CONTROL_ALLOW_ORIGIN,
            allowedOrigin);
    return new GatewayResponse<>(
        EMPTY_BODY, responseHeaders, exception.getStatusCode(), isBase64Encoded, objectMapper);
  }
  ...
  /**
   * Method for sending error messages in case of failure. It can be overriden but it should not be
   * necessary in the general case. It returns to the API-client a specified status code, the
   * message of the exception and optionally another additional message.
   *
   ...
   * @throws GatewayResponseSerializingException when the writer throws an
   *     GatewayResponseSerializingException.
   */
  private GatewayResponse<ThrowableProblem> createResponseReportingProblemToClient(
      Exception exception, Integer statusCode, String requestId)
      throws GatewayResponseSerializingException {
    ThrowableProblem problem = createProblemDescription(exception, statusCode, requestId);
    return new GatewayResponse<>(
        problem, getFailureHeaders(), statusCode, isBase64Encoded, objectMapper);
  }
  ...
  private ThrowableProblem createProblemDescription(
      Exception exception, Integer statusCode, String requestId) {
  ...
  private List<ValidationError> getErrors(Exception exception) {
    return exception instanceof WithErrors withErrors && !withErrors.getErrors().isEmpty()
