...
public class ApiMessageParserTest {
  ...
  private static final Path BODY_NON_JSON_STRING =
      Path.of(API_GATEWAY_MESSAGES_FOLDER, "bodyIsNonJsonString.json");
  private static final Path BODY_JSON_STRING =
      Path.of(API_GATEWAY_MESSAGES_FOLDER, "bodyIsAJsonString.json");
  private static final Path BODY_JSON_ELEMENT =
      Path.of(API_GATEWAY_MESSAGES_FOLDER, "bodyIsAJsonElement.json");
  ...
  public void getRequestInfoThrowsApiIoExceptionWhenParserThrowsAnException()
    ...
    ApiIoException exception =
        assertThrows(ApiIoException.class, () -> parser.getRequestInfo(notImportantInput));
    assertThat(
        exception.getMessage(), containsString(ApiMessageParser.COULD_NOT_PARSE_REQUEST_INFO));
  }
  ...
  public void getRequestInfoThrowsApiIoExceptionWhenInputStringIsNotAJsonString()
    ...
    ApiIoException exception =
        assertThrows(ApiIoException.class, () -> parser.getRequestInfo(notJsonString));
    assertThat(
        exception.getMessage(), containsString(ApiMessageParser.COULD_NOT_PARSE_REQUEST_INFO));
  }

  @DisplayName(
      "getRequestInfo includes the request info string in the exception message when an exception"
          + " in thrown")
  @Test
  public void
      getRequestInfoIncludesTheRequestInfoStringInTheExceptionMessageWhenAnExceptionIsThrown()
          throws JsonProcessingException {
  ...
  @Test
  public void getBodyElementFromJsonReturnsEmptyStringForEmptyBodyAndStringParser()
      throws IOException {
  ...
  private ApiMessageParser<String> parserWithMapperThatThrowsIoException()
    ...
    when(mapper.readValue(anyString(), any(Class.class)))
        .thenThrow(new JsonMappingException(null, SOME_EXCEPTION_MESSAGE));
