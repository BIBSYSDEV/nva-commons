...
class SecretsWriterTest {
    ...
    @Test
    void updateSecretLogsErrorWhenWrongSecretNameIsGiven() {
        final TestAppender appender = LogUtils.getTestingAppender(SecretsReader.class);
        Executable action = () -> secretsWriter.updateSecret(WRONG_SECRET_NAME, SECRET_KEY);
    ...
    @Test
    void updateSecretLogsErrorWhenWrongSecretKeyIsGiven() {
        final TestAppender appender = LogUtils.getTestingAppender(SecretsReader.class);
        Executable action = () -> secretsWriter.updateSecret(SECRET_NAME, WRONG_SECRET_KEY);
    ...
    @Test
    void updateSecretLogsErrorLogsErrorCauseButMasksErrorCauseToCaller() {
        Executable action = () -> secretsWriter.updateSecret(SECRET_NAME, WRONG_SECRET_KEY);
    ...
    @Test
    void updateSecretReturnsSecretValueWhenSecretNameAndSecretKeyAreCorrect() throws ErrorReadingSecretException {
        var value = secretsWriter.updateSecret(SECRET_NAME, SECRET_KEY);
    ...
    void shouldReturnInputSecretAsInputClassWhenInputSecretExistsAndIsStructuredLikeInputSecretClass() {
        var credentials = secretsWriter.updateClassSecret(JSON_SECRET_NAME, Credentials.class);
    ...
    void shouldNotExposeSecretInThrowWhenInputSecretExistsButIsPlaintext() {
        Executable action = () -> secretsWriter.updateClassSecret(PLAIN_TEXT_SECRET_NAME, Credentials.class);
    ...
    void shouldNotExposeSecretInThrowWhenInputSecretExistsButIsJsonList() {
        Executable action = () -> secretsWriter.updateClassSecret(JSON_LIST_SECRET_NAME, Credentials.class);
        var thrown = assertThrows(ErrorReadingSecretException.class, action);
        assertThat(thrown.getMessage(), not(containsString(PLAIN_TEXT_SECRET_VALUE)));
    }

    @Test
    void updatePlainTextSecretWhenSecretNameIsCorrect() {
        var value = secretsWriter.updateSecret(PLAIN_TEXT_SECRET_NAME,PLAIN_TEXT_SECRET_VALUE);
    ...
    private PutSecretValueResponse createGetSecretValueResult(String secretString) {
